...
const app = {
    ...
    if (isAdj) {
      ...
      pool.forEach(rank => {
        ...
        for (let i = 0; i < remaining; i++) {
          weightedPool.push({ rank, bucket: ['J', 'Q', 'K'].includes(rank) ? '10' : rank });
        }
      });
  ...
  removePlayer(hi, i) {
    ...
    if (state.isSplit && state.playerHands[hi].length === 0) {
      state.isSplit = false;
      state.playerHands = [state.playerHands[1 - hi] || []];
      state.activeHand = 0;
      state.splitBets = [];
  ...
  undo() {
    ...
    if (state.isSplit && !isPair(state.playerHands[0])) {
      state.isSplit = false;
      state.playerHands = [state.playerHands[0].concat(state.playerHands[1] || [])];
      state.splitBets = [];
      state.doubleDownHands = [];
    }
    renderUI();
    ...
    if (state.isSplit) {
      ...
      if (outcomes.length > 0) {
        ...
        state.sessionLog.push({
          ...
          playerTotal: Math.max(...pHands.map(h => handTotal(h))),
          dealerCards: [...dHand],
      ...
      if (outcome) {
        const bet = state.doubleDownHands.includes(0) ? (state.confirmedBet?.bet || calcSuggestedBet().bet) * 2 : (state.confirmedBet?.bet || calcSuggestedBet().bet);
