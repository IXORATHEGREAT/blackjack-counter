...
function showBetAdjPopup(type) {
  ...
  const multiplied = type === 'double' ? base * 2 : base * 2; // Both split and double are 2x total
...
// FIXED: Return object with both result amount and bet amount for logging
function calculateHandResult(outcome, bet, isDoubled) {
  const effectiveBet = isDoubled ? bet * 2 : bet;
  if (outcome === 'win') return { amount: effectiveBet, betForLog: effectiveBet };
  if (outcome === 'loss') return { amount: -effectiveBet, betForLog: effectiveBet };
  return { amount: 0, betForLog: 0 }; // Push: 0 result, log bet as 0 per spec
}
