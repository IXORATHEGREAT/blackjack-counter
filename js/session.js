...
// Each hand: {R}{BET36}+{CUM36} or {R}{BET36}~{CUM36}_{PLAYER_CARDS}_{DEALER_CARDS}
// Sign is '+' (positive) or '~' (negative, tilde avoids collision with '-' part delimiter)
function encodeSeed() {
  if (!state.sessionLog.length) return state.sessionSeedPrefix;
  // Use the bankroll that was set when the shoe started, not the live input value
  // This prevents mid-session edits to the bankroll field from corrupting the seed
  const bankroll = state.startingBankroll || parseFloat(document.getElementById('bankroll').value) || CONFIG.STARTING_BANKROLL;
  ...
  try {
    ...
    if (parts.length >= 3) {
      ...
      for (const entry of entries) {
        ...
        // Parse bet and cumulative — sign is '+' (positive) or '~' (negative)
        // '~' used instead of '-' to avoid collision with the '-' part delimiter in the seed string
        const remaining = resultPart.slice(1);
        const plusIdx = remaining.indexOf('+');
        const tildeIdx = remaining.indexOf('~');
        const signIdx = plusIdx > 0 ? plusIdx : tildeIdx > 0 ? tildeIdx : -1;
...
function loadSession() {
  try {
    ...
    state.startingBankroll = data.startingBankroll || CONFIG.STARTING_BANKROLL;
...
function newShoe() {
  ...
  // FIXED: Properly initialize played with buckets
  initPlayed();
  renderUI();
  saveSession();
...
function resetHand() {
  ...
  state.history = state.history.filter(h => h.type !== 'dealer' && h.type !== 'split');
  state.dealerHand = [];
  state.playerHands = [[]];
  state.activeHand = 0;
  state.isSplit = false;
